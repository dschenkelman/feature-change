...
module.exports = function(options, done){
  ...
  if (!options || typeof options !== 'object') {
    return setImmediate(done, new Error('\'options\' argument is invalid'));
  }

  if (typeof options.expected !== 'function') {
    return setImmediate(done, new Error('\'options.expected\' argument must be a function'));
  }

  if (typeof options.actual !== 'function') {
    return setImmediate(done, new Error('\'options.actual\' argument must be a function'));
  }

  if (typeof options.logAction !== 'function') {
    return setImmediate(done, new Error('\'options.logAction\' argument must be a function'));
  }  

  if (options.areEqual && typeof options.areEqual !== 'function') {
    return setImmediate(done, new Error('\'options.areEqual\' argument should be a function'));
  }  
